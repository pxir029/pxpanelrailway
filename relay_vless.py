...
RELAY_BUF = 512 * 1024   # 512 KB — better throughput
...
# Batched traffic accounting — avoids lock on every tiny packet
_pending_bytes: dict = {}
_PENDING_FLUSH = 64 * 1024  # flush every 64KB

async def _flush_pending(uid: str | None = None):
    async with LINKS_LOCK:
        items = list(_pending_bytes.items()) if uid is None else [(uid, _pending_bytes.get(uid, 0))]
        for u, n in items:
            if not n:
                continue
            _pending_bytes[u] = 0
        ...
        if uid is not None and uid in _pending_bytes and _pending_bytes[uid] == 0:
            _pending_bytes.pop(uid, None)
...
async def check_and_use(uid: str, n: int) -> bool:
    ...
    if not is_link_allowed(link):
        return False
    # soft quota check without lock
    limit = int(link.get("limit_bytes") or 0)
    used = int(link.get("used_bytes") or 0) + int(_pending_bytes.get(uid, 0))
    if limit > 0 and used >= limit:
        await _flush_pending(uid)
        link = LINKS.get(uid)
        if not link or not is_link_allowed(link):
            return False
    _pending_bytes[uid] = int(_pending_bytes.get(uid, 0)) + n
    if _pending_bytes[uid] >= _PENDING_FLUSH:
        await _flush_pending(uid)
    return True
