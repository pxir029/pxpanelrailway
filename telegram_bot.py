...
def _welcome_text() -> str:
    # enhanced professional welcome
    ver = globals().get("BOT_VERSION", "14.0.0")
    return (
        f"🤖 <b>{BOT_NAME}</b> · v{ver}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"مدیریت کامل پنل از داخل ربات\n"
        f"• ساخت / حذف / فعال‌سازی کانفیگ\n"
        f"• گروه‌های ساب و آمار زنده\n"
        f"• تنظیمات امنیتی و force-join\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"از منوی زیر انتخاب کنید:"
    )
    # original follows for compatibility - unreachable after return
def __welcome_text_legacy() -> str:
    return (
...
# Multi-user bots registry (panel-managed)
_USER_BOTS_RUNTIME: dict = {}

async def register_user_bot(bot_rec: dict):
    """Register a user-owned bot token for future polling/webhook extension."""
    bid = str(bot_rec.get("id") or "")
    if not bid:
        return
    _USER_BOTS_RUNTIME[bid] = dict(bot_rec)
    logger.info("user bot registered: %s", bot_rec.get("label"))


async def unregister_user_bot(bid: str):
    _USER_BOTS_RUNTIME.pop(str(bid), None)
